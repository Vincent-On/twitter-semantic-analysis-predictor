...
from tweeter_collection import TweeterCollection
"""
Data Visualization Module
"""

stop_words_base = ["0o", "0s", "3a", "3b", "3d", "6b", "6o", "a", "a1", "a2", "a3", "a4", "ab", "able", "about",
                   ...
                   "ys", "yt", "z", "zero", "zi", "zz"]
# You can add any extra words you want to add to stop word list below
stop_words_append = ["https", "nhttps", "xa0", "day", "today", "year", "years", "continue", "time", "people"]
# Master stop word list
stop_words_master = stop_words_base + stop_words_append


def get_dataframes():
    """
    Get the dataframes from their respective csv files.

    :return: a tuple, contains 3 pd.DataFrame objects: left wing set, right wing set, merged set.
    """
    left_df = pd.read_csv("datasets/left_set.csv")
    right_df = pd.read_csv("datasets/right_set.csv")
    master_df = pd.read_csv("datasets/custom_set.csv")
    return left_df, right_df, master_df


def word_counter(tweets: list, stop_words: list):
    """
    Returns a DataFrame containing the most frequent 25 words in from the provided tweets excepting stop words.

    :param stop_words: list, list of stop words that will not be included in the count
    :param tweets: list, contains the contents of all tweets to be counted
    :return: pd.DataFrame
    """
    print(f"Total Tweets Analyzed: {len(tweets)}")
    vectorizer = CountVectorizer(stop_words=stop_words, analyzer='word', max_features=25)
    word_list = vectorizer.fit_transform(tweets)
...
def visualization(word_list: pd.DataFrame, plot_title: str):
    """
    Function to visualize word distribution.

    :param plot_title: string , desired title of the plot
    :param word_list: pd.Dataframe
    :return: None
    """
    top_words = pd.DataFrame(word_list.head(20), columns=["Frequency"])
    top_words.sort_values("Frequency", ascending=True).plot(kind="barh")
...
def main():
    left_df, right_df, master_df = get_dataframes()
    left_collection = TweeterCollection(left_df)
    right_collection = TweeterCollection(right_df)
    left_tweets = left_collection.get_all_tweets_tokenized()
    right_tweets = right_collection.get_all_tweets_tokenized()
    print("Left Wing Word Counts: ")
    left_word_counter = word_counter(left_tweets, stop_words_master)
    print("\n")
    print("Right Wing Word Counts: ")
    right_word_counter = word_counter(right_tweets, stop_words_master)
    visualization(right_word_counter, "Top Words on the Right")
    visualization(left_word_counter, "Top Words on the Left")
    return
